  ...
  <div v-if="artist" class="">
          ...
          <div class="row justify-content-center">
              <div class="col-md-6">
                  ...
                  <div class="text-center"> 
                    ...
                    <a class="social-icons twitter" :href="'http://' + artist.twitterHandle" />
                    <a class="social-icons email" :href="'mailto:' + artist.email" />
                  </div>
              </div>
          </div>
      </div>
    ...
    <div class="mt-4 mb-4">
      <div class="col-lg-12 pt-4 pb-4">
          ...
          <p class="text-muted f-14 mb-3">{{artist.bio}}</p>
      </div>

      <div class="col-lg-12  pt-4 pb-4">
          <h3 class="text-dark ">Related Categories</h3> 
          <ul id="tags">
            <li v-for="tag in artist.tags" :key="tag">
              {{ tag }}
            </li>
          </ul>
      </div>
...
export default {
  ...
  mounted() {
    ...
    axios
      .get(this.API_HOST + '/api/artist/details/' + this.artistId )
      .then(response => {
        if (response) {
          this.artist = response.data.artist;
          this.artsCollection = response.data.arts;
        }
      }).catch(err => {
        console.log(err);
